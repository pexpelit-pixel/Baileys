...
export const makeSocket = (config: SocketConfig) => {
	...
	const requestPairingCode = async (phoneNumber: string, customPairingCode?: string): Promise<string> => {
		// Accept a custom pairing code, normalize it (remove whitespace, uppercase). If not provided, generate one.
		const pairingCode = customPairingCode ? customPairingCode.replace(/\s+/g, '').toUpperCase() : bytesToCrockford(randomBytes(5));

		// NOTE: intentionally not enforcing exact-length here to retain legacy behavior
